...
export class SendEmail {
  ...
  async followUp(): Promise<void> {
    ...
    for (const user of users) {
      const { sendTo, text, html } = this.emailTemplate.generateFollowUpEmail(user);
      await this.emailSender.send(sendTo, 'demand payment', html, text);
      await TelegramAPI.sendMessage(`Send Demand Payment mail to ${user.residentFullName}`);
    }
  }
}
